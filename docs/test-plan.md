...
| G3 | **Auth — ANSWERED** | in-page fetch reaches `gql.waveapps.com` with the session cookie | ✅ run 2 — **HTTP 200 cross-origin; the `waveapps` cookie is NOT httpOnly-blocking** |
| G4 | Download chain | `pdfUrl` fetched to `--out` is a real PDF | ❌ run 2 — never reached: the hand-written GraphQL returns `didSucceed: null` (query wrong; `validated: no`). Fix = **capture the real request (teaching mode)** |
...
| ID | Gate | Pass criteria | Status |
|----|------|---------------|--------|
| G9 | End-to-end teach | reset `download-invoice.md` to UI-only → run teaching mode → it captures, classifies, writes `## API` into the one file, validates, lints CLEAN, emits the `TAUGHT` report | ⬜ never run |
